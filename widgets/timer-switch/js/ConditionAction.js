(async () => {
    class ConditionAction extends HTMLElement {
        ...
        conditionStateIdsAndAlias() {
            return vis.binds["timer-switch"].getConditionStateIdsAndAlias(this.getAttribute("widgetid"));
        }
        ...
        createShadowRoot() {
            ...
            shadowRoot.innerHTML = `
				...
				<link rel="stylesheet" href="widgets/timer-switch/css/ConditionAction.css" />
					...
					<div class="condition-header">
						<div>${vis.binds["timer-switch"].translate("condition")}</div>
						 <img class="button delete" src="widgets/timer-switch/img/delete-24px.svg" width="28px"
							height="28px" title="${vis.binds["timer-switch"].translate("removeCondition")}"/>
					</div>
					<div class="condition-body">
						<select id="selectConditionStateId1" required></select>
						<select id="selectSign" required>
							<option value="==">${vis.binds["timer-switch"].translate("equals")}</option>
							<option value="!=">${vis.binds["timer-switch"].translate("equalsNot")}</option>
						</select>
						<div id="withConstant" class="md-switch-container">
							<div class="md-switch-track"></div>
							<div class="md-switch-handle"></div>
							<div class="md-switch-label">${vis.binds["timer-switch"].translate("withConstant")}</div>
						</div>
